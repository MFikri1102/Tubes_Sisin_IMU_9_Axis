...
#define MQTT_TOPIC_ROBOTHAND "diffie-hellman-robot-hand"
#define MQTT_TOPIC_HUMANHAND "diffie-hellman-human-hand"
...
//  Serial.println(otherPartyKey);
  ...
  if (strcmp(topic, MQTT_TOPIC_ROBOTHAND) == 0) {
    sharedSecretKey = compute(otherPartyKey, b, p);
  } else if (strcmp(topic, MQTT_TOPIC_HUMANHAND) == 0) {
...
//  char buffer[20];
//  sprintf(buffer, "%ld", B);
//  mqttClient.publish(MQTT_TOPIC_HUMANHAND, buffer);
}
...
void loop() {
  ...
  if (!mqttClient.connected()) {
    ...
    mqttClient.subscribe(MQTT_TOPIC_ROBOTHAND);
